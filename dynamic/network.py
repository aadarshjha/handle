...
class ResNeXt(nn.Module):
    def __init__(
        ...
        self.layer1 = self.make_layer(block, 128, layers[0], shortcut_type, cardinality)
    ...
    def make_layer(self, block, planes, blocks, shortcut_type, cardinality, stride=1):
        downsample = None
...
class ResNet_LSTM(nn.Module):
    def __init__(self, num_classes=3):
        ...
        self.lstm = nn.LSTM(
            input_size=512, hidden_size=256, num_layers=6, batch_first=True
        )
        self.fc = nn.Sequential(
            nn.Linear(256, 128), nn.ReLU(inplace=True), nn.Linear(128, num_classes)
        )
...
def load_resnext101(config, device):
    ...
    pretrain_dict["state_dict"] = {
        ...
        for key, value in pretrain_dict["state_dict"].items()
    }
...
def load_timesformer(config, device):
    model = TimeSformer(
        img_size=config.sample_size,
        num_classes=3,
        num_frames=config.sample_duration,
        attention_type="divided_space_time",
    )

    pretrain_dict = torch.load(config.pretrain_path)
