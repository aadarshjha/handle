...
class IPN_Dataset(data.Dataset):
    def __init__(
        ...
        self.init_data(
            video_pth, annotation_pth, split, n_samples_per_video, sample_duration
        )
    ...
    def init_data(
        self, root_pth, annotation_pth, split, n_samples_per_video, sample_duration
    ):
            ...
            if n_samples_per_video == 1:
                ...
                if n_samples_per_video > 1:
                    step = max(
                        1,
                        math.ceil(
                            (n_frames - 1 - sample_duration) / (n_samples_per_video - 1)
                        ),
                    )
                else:
    ...
    def load_clip(self, video_dir_pth, frame_idx, sample_duration):
        ...
        for i in frame_idx:
            img_pth = os.path.join(
                video_dir_pth, "{:s}_{:06d}.jpg".format(video_dir_pth.split("/")[-1], i)
            )
